class Main_Part_ConfigSettingsSpell extends Main_Part_Config_Abstract {
    ...
    static nicknameSets = {
        "lane-nickname": ["TOP", "JG", "MID", "AD", "SUP"],
        "number-nickname": ["P1", "P2", "P3", "P4", "P5"],
        "custom-nickname": ["", "", "", "", ""],
    };
    ...
    ReloadContent = ()=>{
        ...
        return Promise.all([
            ...
            let previewers = $(this.element).find(".config-group[data-name='preview'] .preview-item").get();
            let seperator = " | ";
            ...
            previewers.forEach((previewItem, previewIdx)=>{
                ...
                let getConfig = (path, key, alt)=>((spellConfig[path]||{})[key]||alt);

                let playerNick = randomChampionNames[previewIdx];
                let sendOptionsChampionName = getConfig("settings/spell/send/options.json", "champion-name", 0);
                if(!sendOptionsChampionName) playerNick = getConfig("settings/spell/overall/nickname.json", previewKey, previewKey);

                let randomSpellIds = Object.keys(summonerSpells).sort(()=>(0.5-Math.random())).slice(0, spellHotkeys.length);

                let sendOnlyFlash = getConfig("settings/spell/send/options.json", "only-flash", 0);
                let sendOnlyInCooldown = getConfig("settings/spell/send/options.json", "only-incooldown", 0);
                let sendFormatCooldownG = getConfig("settings/spell/send/format.json", "game-time", 0);
                let sendFormatCooldownM = getConfig("settings/spell/send/format.json", "cooldown-m", 0);
                let sendFormatCooldownS = getConfig("settings/spell/send/format.json", "cooldown-s", 0);
                ...
                let spellString = spellHotkeys.map((hotkey, idx)=>({
                    ...
                    let spellNick = data[Object.keys(data).filter((k)=>getConfig("settings/spell/send/nickname.json", k, 0))[0]||"key"];
