...
class AppBodyBlock_Abstract {
    ...
    LoadComponents = (loading)=>{
        ...
        return Promise.resolve(loading.sort((a,b)=>(a[3]-b[3]))).then((sorted)=>{
            let load = (idx)=>{
                if(idx >= sorted.length) return Promise.resolve();
                this.components[sorted[idx][0]] = new sorted[idx][1](this.container, sorted[idx][2], sorted[idx][3]);
                return this.IsComponentReady(this.components[sorted[idx][0]]).then(()=>load(idx+1));
            };
            return load(0);
        }).then(()=>this.SlideIn(((c)=>{
    ...
    AddComponent = (name, _class, data, index=0)=>{
        let direction = Object.getPrototypeOf(this).constructor.animationDirection;
        return this.SlideOut((c)=>(c.index>=index)).then(()=>{
            this.components[name] = new _class(this.container, data, index);
    ...
    reloadPage = ()=>{
        if(window.Side && window.Main){
            ...
            SummonerIdentifier.CurrentSummoner().then((identifier)=>{
                window.LoadSide(sideConstructor, window.MakeData({summoner:identifier}));
            });
