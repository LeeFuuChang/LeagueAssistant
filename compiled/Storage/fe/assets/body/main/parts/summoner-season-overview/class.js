class Main_Part_SummonerSeasonOverview extends AppBodyMain_Part {
    ...
    ReloadContent = ()=>{
        ...
        new Promise((resolve, reject)=>{
            ...
            $(this.element).find(".player-banner .profile .basic .info .name").text(summoner["displayName"]);
            ...
            $.get("/riot/lcu/0/lol-replays/v1/configuration", {}, (configurationRequest)=>{
                ...
                let displaySummaries = (seasonMatchesSummaries)=>{
                    ...
                    let statsSummaryKDA = (statsSummary["kills"]+statsSummary["assists"])/Math.max(statsSummary["deaths"], 1);
                    ...
                    let loadStatsSummary = (queueData, queueName)=>{
                        ...
                        let queueWR = (Ws/Math.max(Ws+Ls, 1))*100;
                    ...
                    for(let position of Object.keys(positionSummary)){
                        let positionItem = $(this.element)
                        .find(`.player-stats .player-stats-inner .recent .positions .item[data-name="${position}"]`);
                        positionItem.find(".pr span").text(((positionSummary[position]/Math.max(totalGamesCount, 1))*100).toFixed(1));
                    ...
                    for(let i=0; i<Math.min(recentChamps.length, 3); i++){
                        ...
                        let championKDA = (
                            recentChamps[i]["kills"]+recentChamps[i]["assists"]
                        ) / Math.max(recentChamps[i]["deaths"], 1);
