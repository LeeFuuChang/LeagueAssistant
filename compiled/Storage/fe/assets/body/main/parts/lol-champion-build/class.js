class Main_Part_LolChampionBuild extends AppBodyMain_Part {
    ...
    FetchStaticData = ()=>{
        ...
        let itemIconsPromise = new Promise((resolve, reject)=>{
            let requestURL = "https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/default/v1/items.json";
            $.get(requestURL, {}, (data)=>resolve(Object.fromEntries(data.map((i)=>[parseInt(i.id),window.ToCDragonPath(i["iconPath"])]))));
        });
    ...
    ReloadContent = ()=>{
        ...
        switch(gameMode.attr("data-label")){
            ...
            default:
                requestURL = `/opgg/lol/champions/${
                    ...
                    (identifierPosition?`${identifierPosition.toLowerCase()}/build?`:"")
                }?region=global&tier=${
        ...
        return $.get(requestURL, {}, (pageProps)=>{
            ...
            this.SetDefault().then(()=>this.FetchStaticData()).then(([
                championSummary,
                itemIcons,
                perkIcons,
                summonerSpellIcons,
            ])=>{
                $(this.element).find(".champion-banner .champion-info .champion-name").text(championSummary[championId]["name"]);
                ...
                try{ // champion-duo
                    ...
                    let fetchData = (currentDate, fetched, count, depth, maxDepth)=>{
                        ...
                        return new Promise((resolve, reject)=>{
                            ...
                            $.get("/qq/lol/getRankDouble", {
                                ...
                                return resolve(fetched.concat(data["data"]));
                            });
                    ...
                    fetchData(new Date(), [], duo_champions.length, 0, 9).then((duos)=>{
                        duos.sort((a,b)=>(b["doublewinrate"]-a["doublewinrate"])).forEach((data, idx)=>{
                            ...
                            duo_champion.on("click", function(){OnChampionSelected(this, componentIdentifier)});
                        });
                    });
                }catch(e){console.log(e)}
                ...
                try{ // champion-match
                    ...
                    counters.slice(0,cell_match_hard_champions.length).forEach((data, idx)=>{
                        ...
                        hard_champion.find(".icon").html(`<img src="
                            https://cdn.communitydragon.org/latest/champion/${data["id"]}/square
                        " alt="">`);
                        ...
                        let componentIdentifier = this.data["identifier"];
                        let OnChampionSelected = this.data["functions"]["OnChampionSelected"];
                        hard_champion.on("click", function(){OnChampionSelected(this, componentIdentifier)});
                    });
                    ...
                    counters.slice(0-cell_match_easy_champions.length).reverse().forEach((data, idx)=>{
                        ...
                        easy_champion.find(".icon").html(`<img src="
                            https://cdn.communitydragon.org/latest/champion/${data["id"]}/square
                        " alt="">`);
                        ...
                        let componentIdentifier = this.data["identifier"];
                        let OnChampionSelected = this.data["functions"]["OnChampionSelected"];
                        easy_champion.on("click", function(){OnChampionSelected(this, componentIdentifier)});
                    });
                }catch(e){console.log(e)}
            });
        });
