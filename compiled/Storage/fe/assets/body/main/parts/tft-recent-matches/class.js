class Main_Part_TftRecentMatches extends AppBodyMain_Part {
    ...
    ReloadContent = ()=>{
        ...
        let tftCompanionsPromise = new Promise((resolve, reject)=>{
            let tftCompanions = {};
            let tftCompanionsRequestURL = "https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/default/v1/companions.json";
            $.get(tftCompanionsRequestURL, {}, (data)=>{
                for(let c of data) tftCompanions[c["contentId"]] = c;
                resolve(tftCompanions);
            });
        });
        ...
        Promise.all([
            ...
            $.get("/riot/lcu/0/lol-tft-team-planner/v1/team/dirty", {}, (tftPlannerRequest)=>{
                ...
                $.get(requestURL, {...pageData.value, "tag":latestSetTag}, (data)=>{
                    if(data["success"]){
                        container.empty();
                        ...
                        for(let i=0; i<games.length; i++){
                            let player = games[i]["json"]["participants"].sort((a, b)=>{
                                return (b["puuid"]===puuid)-(a["puuid"]===puuid);
                            })[0];
    
                            let ele = this.CreateInnerItem(container);
                            ele.attr("data-id", games[i]["json"]["game_id"]);
                            ele.attr("win", +(player["placement"]*2 < games[i]["json"]["participants"].length));
